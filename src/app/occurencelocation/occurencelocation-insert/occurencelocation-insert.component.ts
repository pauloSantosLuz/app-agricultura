...
import { OccurenceService } from 'src/app/occurence/occurence.service';
...
@Component({
  selector: 'app-occurencelocation-insert',
  templateUrl: './occurencelocation-insert.component.html',
  styleUrls: ['./occurencelocation-insert.component.css']
})
export class OccurencelocationInsertComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //pega os valor  
    this.idOcorrencia = this.activateRoute.snapshot.params['id'];
    this.ocorrenciaDesc = this.activateRoute.snapshot.params['ocorrenciaDesc'];
  ...
  add() {
    ...
    if (coordenadas == "") {
      ...
      this.occurenceService.postOccurenceLocation("SRID=4326;GEOMETRYCOLLECTION("+ coordenadas+")",this.f.observacao.value,this.idOcorrencia).subscribe(
        () => {
